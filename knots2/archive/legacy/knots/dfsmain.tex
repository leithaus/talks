...
%\begin{frontmatter
\emph{\title{Knots as processes: knot theory embeds in the $\pi$-calculus}
...
homology theory \cite{DBLP:journals/entcs/HerlihyRT02}. The authors, exploiting a duality hidden within that framework,
have switched the roles of the domains
and the
...
the introduction of a relation (in addition to equivalence) called \textit{reduction} that is
neither \textit{a priori} reflexive or symmetric, representing the evolution or
...
where dynamics are typically described as functionals.

It is noteworthy that several other `computational calculi' also admit
similar algebraic presentations. The family of lambda calculi
provide a canonical example, presenting: term grammar as generalized generators;
alpha-equivalence as relations; and beta-reduction as the explicit
account of dynamics. We submit that, by utilizing
bisimulation techniques, some of these
computational calculi may generally constitute an
interesting mine for
topological invariants. This paper provides evidence as to this last statement.
...
\section{Knots and their representations}

% This is not a good intro to this section for%%
% the knots journal 

In this section, we review a bit of knot theory and
establish some connections useful for proving our lemmas 
and theorems. By a
\textit{knot}, we mean an embedding (either smooth or piecewise linear), up to isotopy
...
primary interest in knot theory. Different ways of representing knots suggest different ways of approaching the fundamental question.

\subsection{A short history of knot diagrams} 

\input{4plathistory.tex}

\section{Reidemeister moves}\label{sec:reidemeister_moves}


There are several ways to represent a knot. The most common way of
representing a knot is through a carefully chosen 
orthogonal projection of the knot onto a plane, where any
crossing point in the resulting diagram (the knot projection) corresponds to at
most two points of the knot
\cite{LivingstonText}. Such a diagram is clearly not unique, as it depends on the plane projected upon.  Reidemeister had the insight that
...
knot diagram. In particular, any knot projection of a knot can be obtained from any other knot projection of the same knot \emph{via} a finite sequence of Reidemeister moves.
...
process, $\meaningof{C(i)}_{\pi}(x_0,x_1,y_0,y_1,u)$, parameterized in
4~+~1
...
perspective, the encoding is shaped as follows.

\begin{equation}
  \meaningof{K}_{\pi} = (v_0 ... v_{2n-1}) \Pi_{i = 0}^{n-1} (\nu \; u)\meaningof{C(i)}_{\pi}(v_{\omega(i,0)},...,v_{\omega(i,3)},u)
\end{equation}

where $n$ is the crossing number of $K$ and 
the function $\omega:
\integers_{n} \times \integers_{4} \to
\integers_{2n}$ gives the index into the list of
ports, $v_0 ... v_{2n-1}$ used
to wire the crossing circuits together in a manner consistent with the
chosen knot diagram of $K$. In particular,
$\omega(i,j)$ gives the index of the $v_{k}$ to
which the $j$th port of the $i$th crossing is
identified. We
now give an example to clarify the discussion.

\subsection{Example: encoding the trefoil as a
process}
Before giving a formal account of the general encoding
we illustrate the principle ideas by encoding the
trefoil knot as a process.
...
\begin{figure}
\centering
\includegraphics[width=2.5in]{dfsTrefoilMethodIllustration.pdf}
...
$y_0$. In particular, it must somehow capture what
it means for a wire to
...
synchronization channel. To alert the under-crossing wire that
it is free to proceed, the over-crossing wire must
fire off an output. Thus, a
crossing circuit is coded as the following
process, where $x_{1}$ and $y_{0}$ are the ports
of the over-crossing wire and $x_{0}, y_{1}$ are
the ports for the under-crossing wire (see Fig.
\ref{trefoil}).

% We desperately need to show where the critical%%
% overcrossing is! Otherwise, the crossing%%
% appears to the untrained eye as symmetric
...
The function $\omega:\integers_{3}\times
\integers_{4}\to \integers_{6}$ maps each port
at a given crossing to one of $v_{0}, \ldots,
v_{5}$. In Fig. \ref{trefoil} we've labeled the bottom-most crossing as crossing number $i=0$ and counted counterclockwise from
there. So for example
$\omega(2,0)=v_{6}$, since the port into the third
under-crossing  (from a counterclockwise
perspective at least) is fed by wire $v_{6}$.




% would help to have omega explicitly

\subsection{The general encoding}

As we have already seen in the example how to encode
a crossing
...
ordering
     implied by the DT code, $DT(K)$. The constraints in Fig. \ref{constraints} uniquely determine a wiring of the
     ...
     \begin{figure}
       \begin{eqnarray}
         ...
         x1(C(i)) & = & y1(C(p(i)+1))
       \end{eqnarray}
       \caption{DT-constraints}\label{constraints}
   \end{figure}
